...
function validateScenario(body) {
  ...
  if (!isFinite(drs) || drs < 0 || drs > 100)
    problems.push("defaultRateShock must be 0-100 (percentage points)");
  const ext = Number(body.tenorExtensionDays);
  if (!isFinite(ext) || ext < 0 || ext > 365)
    problems.push("tenorExtensionDays must be 0-365");
  const hc = Number(body.recoveryHaircut);
  if (!isFinite(hc) || hc < 0 || hc > 1)
    problems.push("recoveryHaircut must be 0-1");
  return problems;
}
...
exports.getScenario = async (req, res) => {
  try {
    const r = await pool.query("SELECT * FROM stress_scenarios WHERE id = $1", [
  ...
  try {
    ...
    const r = await pool.query(
      `
        ...
        `,
      [
        funderId ? String(funderId) : null,
        String(name).trim(),
        description ? String(description).trim() : null,
        Number(defaultRateShock),
        Math.round(Number(tenorExtensionDays)),
        Number(recoveryHaircut),
      ],
    );
...
exports.deleteScenario = async (req, res) => {
  try {
    const r = await pool.query(
      "DELETE FROM stress_scenarios WHERE id = $1 RETURNING id",
      [Number(req.params.id)],
    );
    if (r.rows.length === 0)
      return res.status(404).json({ error: "Scenario not found" });
...
function stressHolding(h, scenario) {
  const stressedPD = clamp(
    h.pdAnnual + Number(scenario.default_rate_shock) / 100,
    0,
    1,
  );
  const stressedDays = h.holdingDays + Number(scenario.tenor_extension_days);
  const baseRecovery = 1 - h.lgd;
  const stressedLGD = clamp(
    1 - baseRecovery * (1 - Number(scenario.recovery_haircut)),
    0,
    1,
  );
  const stressedLoss =
    h.principal * (stressedPD * (stressedDays / 365)) * stressedLGD;
...
exports.runStress = async (req, res) => {
  try {
    ...
    if (!funderId)
      return res.status(400).json({ error: "funderId is required" });
    if (!scenarioId)
    ...
    if (holdings.length === 0) {
      return res
        .status(400)
        .json({ error: "No active holdings to stress for this funder" });
    }
    ...
    const saved = await pool.query(
      ...
      [
        ...
        Math.round(baseEL),
        Math.round(stressedEL),
        Math.round(baselineRiskAdjusted),
        Math.round(stressedRiskAdjusted),
        survives,
      ],
    );
    ...
    if (!survives) {
      try {
        await notificationService.sendNotification({
          ...
          message:
            `Stress test "${scenario.name}" would push the portfolio to a net loss of ` +
    ...
    res.status(200).json({
      ...
      returnErosionPct:
        baselineRiskAdjusted !== 0
          ? Number(
              (
                ((baselineRiskAdjusted - stressedRiskAdjusted) /
                  Math.abs(baselineRiskAdjusted)) *
                100
              ).toFixed(1),
            )
          : 0,
...
exports.listRuns = async (req, res) => {
  try {
    ...
    const result = funder
      ? await pool.query(
          "SELECT * FROM stress_runs WHERE funder_id = $1 ORDER BY created_at DESC LIMIT 50",
          [funder],
        )
      : await pool.query(
          "SELECT * FROM stress_runs ORDER BY created_at DESC LIMIT 50",
        );
    res.status(200).json(result.rows);
  } catch (error) {
